...
# Modular PostgreSQL Replay Data Analytics Pipeline
...
*   **Player Overall Average Lobby Skill (default mode if no args):**
    ...
    ```
    *With a custom "since date" filter (e.g., only players active after January 1, 2024):*
    ```bash
...
*   **`main.py`**: The minimalist entry point. Its *sole* responsibility is to initiate the CLI parsing and then dispatch the application's core logic.
